...
import { MEAL_TYPES, EXTRA_CHARGES, FRESHUP_TIMES } from '@utils/constants';
...
export const AppContent: React.FC = () => {
  ...
  const handleCalculate = () => {
    ...
    try {
      const { breakdown } = CalculationEngine.calculateBreakdown(
        basicInfo,
        vehicleSelection.type,
        vehicleSelection.quantity,
        itinerary,
        accommodation,
        freshups,
        meals,
      ...
      const newInvoice = CalculationEngine.generateInvoice(
        basicInfo,
        vehicleSelection.type,
        vehicleSelection.quantity,
        breakdown
  ...
  return (
    <div className="app">
      <PageHeader
        title="Joshy's Tour Planner"
        subtitle="Student Trip Enquiry & Live Cost Estimation System"
      />

      <Step01BasicInfo onGeneratePlan={handleGeneratePlan} />
      <Step02VehicleSelection />
      <Step03Itinerary />
      <Step04Accommodation />
      <Step05Freshups />
      <Step06Meals />
      ...
      <div style={{ marginBottom: '2rem' }}>
        <Button variant="primary" onClick={handleCalculate}>
          <i className="fa-solid fa-calculator"></i> Calculate Total Trip Cost & Generate Invoice
        </Button>
      </div>

      <InvoiceComponent invoice={invoice} visible={invoiceVisible} />
    </div>
  );
};
